...
from aod.aod_core import load_checkpoint
from aod.hallusionbench_layers import load_layer_dataset, split_indices
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    x_te = ds.x[test_idx]
    y_te = ds.y_gt[test_idx]
    y_base_te = ds.y_pred_base[test_idx]
    base_acc = _acc_binary(y_base_te, y_te)

    model.eval()
    with torch.no_grad():
        out = model(x_te)
        probe_acc = _acc_from_logits(out["pred_truth"], y_te)
        h_hallu_te = out["h_hallu"]
        ...
        best_acc = float("-inf")
        for a in alphas:
            x_int = x_te - float(a) * h_hallu_te
            logits = model.truth_probe(x_int)
            acc = _acc_from_logits(logits, y_te)
            if acc > best_acc:
                best_acc = float(acc)
                best_alpha = float(a)
            print(f"[alpha={a:g}] acc={acc:.2f}%")

    print("\n=== HallusionBench AOD evaluation ===")
    print(f"ckpt={args.ckpt}")
    print(f"layer={layer} n_test={int(test_idx.size)} base_acc={base_acc:.2f}% probe_acc={probe_acc:.2f}%")
    print(f"best_alpha={best_alpha:g} best_acc={best_acc:.2f}% gain(best-base)={best_acc-base_acc:+.2f}%")
    return 0
